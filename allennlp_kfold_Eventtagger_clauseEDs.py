...
fold = 10
dataset = "InScript_ganzeSaetze_train.txt" 
...
instanzen = dict()
zeile = 0
with codecs.open(dataset,"r","utf-8",errors='ignore') as f:
    for line in f:
        instanzen[zeile]=line.strip()
        zeile+=1
        
reader = EDDatasetReader({"bert" : PretrainedBertIndexer(pretrained_model="bert-base-uncased",truncate_long_sequences=False)})
...
for i in range(0,fold):
    ...
    lstm = PytorchSeq2SeqWrapper(torch.nn.LSTM(hidden_size = HIDDEN_DIM1,input_size=word_embeddings.get_output_dim(),batch_first=True,bidirectional=True))
    lstm2 = PytorchSeq2SeqWrapper(torch.nn.LSTM(hidden_size = HIDDEN_DIM2,input_size=lstm.get_output_dim(),batch_first=True,bidirectional=True))
    model = LstmTagger(word_embeddings,lstm,lstm2,vocab)
